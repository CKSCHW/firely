...
import { getDevices, getPlaylists, getContentItems } from '@/data/mockData';
import { useEffect, useState } from 'react';
import type { DisplayDevice, Playlist, ContentItem } from "@/lib/types";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function loadData() {
      setIsLoading(true);
      try {
        const [devices, playlists, contentItems] = await Promise.all([
          getDevices(),
          getPlaylists(),
          getContentItems()
        ]);
        setOnlineDevices(devices.filter(d => d.status === 'online').length);
        setTotalDevices(devices.length);
        setTotalPlaylists(playlists.length);
        setTotalContentItems(contentItems.length);
      } catch (error) {
